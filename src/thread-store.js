...
// Legacy key — the delete confirmation's "don't show this again" preference is no longer persisted,
// but a browser that used an earlier build may still carry it, so every load clears it.
const SKIP_DELETE_WARNING_KEY = 'threadsSkipDeleteWarning'
...
function ensure() {
  ...
  try { localStorage.removeItem(SKIP_DELETE_WARNING_KEY) } catch {}   // clear any stale suppression
